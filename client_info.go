...
type ServerStatus struct {
	...
	GPUInfo      []status.GPUUsage `json:"gpu_info"`
	GPUmeminfo   float64           `json:"gpu_mem_info"`
...
func getServerStatus() ServerStatus {
	...
	// gpu info
	gpuInfo, err := status.GetGpuUtils()
	if err != nil {
		log.Println("GetGpuUtils error: ", err)
	}
	// fmt.Println(gpuInfo)
	item.GPUInfo = gpuInfo
	var usedGPUMemory int
	var totalGPUMemory int

	for _, v := range gpuInfo {
		totalGPUMemory += v.MemoryTotal
		usedGPUMemory += v.MemoryUsed
	}

	item.GPUmeminfo = float64(usedGPUMemory) / float64(totalGPUMemory)
